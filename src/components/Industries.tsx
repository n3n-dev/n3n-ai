...
import { Building2, Shield, Factory, Zap, Server, Truck, ArrowRight, Plus } from 'lucide-react'
   ...
   <div className="relative max-w-6xl mx-auto px-5 md:px-16 lg:px-24">
    ...
    >
     {(() => {
      ...
      return eyebrowText ? (
       <span className={`text-sm font-semibold tracking-[0.2em] uppercase ${eyebrowClass}`}>
        {eyebrowText}
       </span>
      ) : null
     })()}
     ...
     <p className={`md:hidden text-[14px] max-w-3xl mx-auto leading-[1.6] whitespace-pre-line break-keep ${subClass}`}>
      {subOverride ?? tr.industries.sub}
     </p>
     <p className={`hidden md:block text-[16px] max-w-3xl mx-auto leading-[1.6] whitespace-pre-line break-keep ${subClass}`}>
    ...
    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 ${light ? 'border-t border-r border-gray-200' : ''}`}>
       ...
       >
        ...
        {steps && steps.length > 0 && (
         <div className={`mt-auto pt-4 border-t ${nextBorder}`}>
          <span className={`text-[10px] font-bold tracking-[0.22em] uppercase block mb-2 ${nextLabelClass}`}>
          ...
          <div className="flex flex-col gap-1">
           {steps.map((step, j) => (
            <div key={j} className="flex items-start gap-2">
             ...
             />
             <span className={`text-[13px] font-medium leading-snug break-keep ${nextItemClass}`}>
              {step}
             </span>
            </div>
           ))}
