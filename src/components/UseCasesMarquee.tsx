...
const CATEGORIES = [
 '물류', '도시 운영', '데이터센터', '생산공정', '서비스 운영', '스마트시티',
 'IT 인프라', '산업현장', '공공인프라',
 'Logistics', 'Urban Ops', 'Data Centers', 'Production', 'Service Ops',
 'Smart City', 'IT Infra', 'Industrial', 'Public Infra',
] as const
...
function Card({ item, cta, lightBg }: { item: { client: string; category: string; desc: string; image: string }; cta: string; lightBg?: boolean }) {
 // Layout: outer article is a flex-column with comfortable left padding;
 // CTA is anchored to the bottom via mt-auto so it sits in the same
 // position regardless of how long the description runs.
 ...
 const titleColor = lightBg ? 'text-gray-900' : 'text-gray-900 dark:text-gray-100'
 const descColor = lightBg ? 'text-gray-500' : 'text-gray-500 dark:text-gray-400'
 ...
 return (
  <article className={cardBase}>
   ...
   <h3 className={`relative mt-3 font-semibold tracking-tight text-[15px] ${titleColor}`}>
    {item.client}
   </h3>

   {/* Description, secondary, muted. */}
   <p className={`relative mt-1.5 text-[13px] leading-[1.55] line-clamp-4 ${descColor}`}>
    {item.desc}
   </p>

   {/* CTA, anchored to the bottom of the card so its position is
     identical across cards regardless of body length. */}
   <a
    href="#contact"
    className={`relative mt-auto pt-4 inline-flex items-center gap-1 text-xs font-medium group-hover:gap-1.5 transition-all self-end ${
     lightBg ? 'text-gray-700 hover:text-gray-900' : 'text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white'
    }`}
   >
    {cta}
    <ArrowRight size={12} />
...
function MarqueeRow({ items, cta, direction = 'left', duration = 40, lightBg }: {
 ...
 return (
  <div className="overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_5%,black_95%,transparent)]">
   ...
   >
    {doubled.map((item, i) => (
     <Card key={`${item.client}-${i}`} item={item} cta={cta} lightBg={lightBg} />
    ))}
   </motion.div>
  </div>
 )
...
export default function UseCasesMarquee({ tr, eyebrowOverride, h2Override, subOverride, lightBg }: Props) {
 ...
 return (
  ...
  >
   ...
   <div className="flex flex-col gap-6">
    <MarqueeRow items={row1} cta={tr.usecases.cta} direction="left" duration={45} lightBg={lightBg} />
    {row2.length > 0 && (
     <MarqueeRow items={row2} cta={tr.usecases.cta} direction="right" duration={50} lightBg={lightBg} />
    )}
   </div>
  </section>
 )
